import { TimelineLite, CSSPlugin, AttrPlugin, Sine, Power2, Power1, TweenLite, SlowMo }  from "gsap/all";
...
window.onload=function(){
    ...
    createTxt('Search', searchLbl, 'search-text');
    ...
    function animSearch() {
      animating = true;
      TLSrch.restart()
      .to(searchBrd, .4, {scaleX: 1.75, x: -25, y: -25, ease: Sine.easeIn})
      .to(searchBtn, .4, {rotationX: 90, ease: Sine.easeIn}, '-=.4')
      .to(searchBrd, .4, {rotation: 180, x: '-=6', y: '+=30', ease: Power2.easeInOut})
      .set(searchBrd, {transformOrigin: '0', marginRight: '-38px'})
      .set(search, {className: '+=edge', onComplete: showTxt})
      .to(searchBrd, .6, {scaleX: 100, ease: Power1.easeOut})
